...
/// This will always turn off the RTC and ADC clocks (unless we end up needing those).
/// If stdio is disabled, it will turn off the USB clock and disable the USB PLL (if we also turn off the RTC and ADC clocks).
/// It will also enable certain clocks when sleeping depending on how subsystems are configured.
pub fn configureClocks() void {
    // Turn off RTC and ADC clocks always to save power
    c.clock_stop(c.clk_rtc);
    c.clock_stop(c.clk_adc);

    if (!logger.stdio_enabled) {
        // We don't need stdio (which is only over USB), so disable the USB clock and PLL which the above two clocks
        // also rely on.
